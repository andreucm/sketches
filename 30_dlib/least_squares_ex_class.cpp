...
public:
        quad(){};
        ~quad(){};
        double ff(matrix<double,1,1> xx) const
        {
                return xx(0)*xx(0);
        }
//         double operator() (matrix<double,1,1> xx) const
//         {
//                 return xx(0)*xx(0);
//         }
        double operator() (matrix<double> xx) const
        {
                return xx(0)*xx(0);
        }
...
//same result than () operator, but as a function instead of an operator
double ff (
    const std::pair<input_vector, double>& data,
    const parameter_vector& params
) const
...
int main()
{
    try
    {
        ...
        {
            input = 10*randm(2,1);
            const double output = myFs.model(input, params);
...
//        cout << "derivative error: " << length(myFs.residual_derivative(data_samples[0], params) - 
//                                                derivative(myFs)(data_samples[0], params) ) << endl;
        cout << "derivative value (func()): " << derivative(myFs,1e-7)(data_samples[0], params) << endl;	
        
        p1(0) = 3;
        double derivativeValue1 =  derivative(qq,1e-7)(p1);
        cout << "derivative value (quad()): " << derivativeValue1 << endl;
        
        double derivativeValue2 = derivative([&](matrix<double,1,1> xx){return qq.ff(xx);},1e-7)(p1);
        cout << "derivative value (quad.ff()): " << derivativeValue2 << endl;

//         cout << "derivative value (quad->()): " << derivative(qq.ff,1e-7)(p1) << endl;  
// 	cout << "derivative value: " << derivative(myFs.ff,1e-7)(data_samples[0], params);
/*
	cout << "derivative value: " << trans( derivative(&myFs.residual,1e-7)( data_samples[0], params) );
	//cout << "derivative value: " << trans( derivative(&residual(data_samples[0], params)) );
	//cout << "derivative value: " << trans( derivative<residual>(data_samples[0], params) );
// 	cout << "derivative value: " << trans( derivative<residual>(residual,1e-7)( data_samples[0], params) );
...
*/                               
	solve_least_squares_lm(	objective_delta_stop_strategy(1e-7).be_verbose(), 
        ...
        p1(0) = 3;
        find_min( dlib::lbfgs_search_strategy(10),
                  dlib::objective_delta_stop_strategy(),
                  qq,
                  derivative(qq), 
                  p1,
                  1e-7);
        cout << "find_min: " << p1(0) << endl;
